...
//初始化单例指针
proGit* proGit::m_instance = nullptr;

proGit::proGit(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::proGit)
{
    ui->setupUi(this);
...
proGit* proGit::getInstance()
{
    if(m_instance == nullptr)m_instance = new proGit();
    return m_instance;
}

void proGit::getModelTreeRequest()
{
   QNetworkRequest request;
   request.setUrl(QUrl(g_getBaseUrl() + g_getModeTreeUrl + "?productMappingId=3&schemeLibraryClass=2" ));
...
void proGit::makeStandardGitTree(StandardGitTreeItem* root, QJsonObject jObj)
{
...
void proGit::assignForItemByJobject(StandardGitTreeItem* item, QJsonObject jObject)
{
...
void proGit::getQuery3DTPGitMapList()
{
    QNetworkRequest request;
...
void proGit::getFile(QUrl url, int row)
{
    QNetworkRequest request;
    request.setUrl(url);
    request.setAttribute(QNetworkRequest::RedirectPolicyAttribute, true);
...
void proGit::getOpenModel(QUrl url)
{
    QNetworkRequest request;
    request.setUrl(url);
    request.setAttribute(QNetworkRequest::RedirectPolicyAttribute, true);
...
void proGit::getOpenModeFile(QUrl u, QJsonObject obj)
{
...
void proGit::postCheckOut(QUrl url)
{
    QNetworkRequest request;
    request.setUrl(url);
    request.setHeader(QNetworkRequest::ContentTypeHeader, QVariant("application/json;charset=UTF-8"));
...
void proGit::makeCapGitTree(StandardGitTreeItem* root, QJsonArray arr)
{
...
void proGit::showEvent(QShowEvent *event)
{
    //先获取目录树
    getModelTreeRequest();
...
void proGit::closeEvent(QCloseEvent *event)
{
    if (!this->isEnabled()) {
...
void proGit::on_btnDetail_clicked()
{
    if (m_currentTreeItem == nullptr)return;
    if (m_currentTreeItem->realType.toInt() != 6)return;
    int i = -1;
    ...
    PRT* p = new PRT(m_standardDataModel->data(m_standardDataModel->index(i, 7)).toString(), this);
    p->setWindowFlags(p->windowFlags() | Qt::WindowStaysOnTopHint);
    p->setAttribute(Qt::WA_DeleteOnClose);
    QRect rect = QGuiApplication::primaryScreen()->availableGeometry();
    p->resize(rect.width() / 2, p->height());
    p->show();
}

void proGit::onGetModelTreeFinished()
{
...
void proGit::on_btnOpenModel_clicked()
{
...
void proGit::on_treeViewStandardGit_doubleClicked(const QModelIndex &index)
{
...
void proGit::onGetQuery3DTPGitMaplistFinished()
{
    QNetworkReply* reply = qobject_cast<QNetworkReply*>(sender());
...
void proGit::onGetOpenModelFinshed()
{
    QNetworkReply* reply = qobject_cast<QNetworkReply*>(sender());
    if (reply != nullptr) {
        int statusCode = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
...
void proGit::onGetOpenModelFileFinished()
{
    bool flag = false;
    m_openModelCount++;
    ...
    if (reply->error() != QNetworkReply::NoError) {
        ...
        if (g_currentWorkDir.isEmpty()) {
            file.setFileName(reply->property("fileName").toString());
        }
        else {
            file.setFileName(g_currentWorkDir + "\\" + reply->property("fileName").toString());
        }
...
void proGit::onPostCheckOutFinished()
{
    QNetworkReply* reply = qobject_cast<QNetworkReply*>(sender());
