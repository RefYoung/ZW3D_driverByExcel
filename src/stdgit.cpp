...
//初始化单例指针
stdGit* stdGit::m_instance = nullptr;

stdGit::stdGit(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::stdGit)
{
    ui->setupUi(this);
...
stdGit* stdGit::getInstance()
{
    if(m_instance == nullptr)m_instance = new stdGit();
    return m_instance;
}

void stdGit::getModelTreeRequest()
{
...
void stdGit::makeStandardGitTree(StandardGitTreeItem* root, QJsonObject jObj)
{
...
void stdGit::assignForItemByJobject(StandardGitTreeItem* item, QJsonObject jObject)
{
...
void stdGit::getQuery3DTPGitMapList()
{
    QNetworkRequest request;
...
void stdGit::getFile(QUrl url, int row)
{
    QNetworkRequest request;
    request.setUrl(url);
    request.setAttribute(QNetworkRequest::RedirectPolicyAttribute, true);
...
void stdGit::getOpenModel(QUrl url)
{
    QNetworkRequest request;
    request.setUrl(url);
    request.setAttribute(QNetworkRequest::RedirectPolicyAttribute, true);
...
void stdGit::getOpenModeFile(QUrl u, QJsonObject obj)
{
...
void stdGit::postCheckOut(QUrl url)
{
    QNetworkRequest request;
    request.setUrl(url);
    request.setHeader(QNetworkRequest::ContentTypeHeader, QVariant("application/json;charset=UTF-8"));
...
void stdGit::showEvent(QShowEvent *event)
{
    //先获取目录树
    getModelTreeRequest();
...
void stdGit::closeEvent(QCloseEvent *event)
{
    if (!this->isEnabled()) {
...
void stdGit::onGetModelTreeFinished()
{
    ...
    if (reply != nullptr) {
        ...
        if (statusCode == 200) {
            ...
            //ui->treeViewStandardGit->expandAll();
        }
    }
}


void stdGit::on_btnOpenModel_clicked()
{
...
void stdGit::on_treeViewStandardGit_doubleClicked(const QModelIndex &index)
{
...
void stdGit::onGetQuery3DTPGitMaplistFinished()
{
    QNetworkReply* reply = qobject_cast<QNetworkReply*>(sender());
...
void stdGit::onGetOpenModelFinshed()
{
    QNetworkReply* reply = qobject_cast<QNetworkReply*>(sender());
    if (reply != nullptr) {
        int statusCode = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
...
void stdGit::onGetOpenModelFileFinished()
{
    bool flag = false;
    m_openModelCount++;
...
void stdGit::onPostCheckOutFinished()
{
    QNetworkReply* reply = qobject_cast<QNetworkReply*>(sender());
